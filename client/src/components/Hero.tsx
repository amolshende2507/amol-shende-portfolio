...
import { useState, useEffect } from 'react';
import { Code, Database, Globe, Cpu, Download } from 'lucide-react';
...
const FloatingIcon = ({
  ...
  <div
    ...
    style={{ animationDelay: `${delay}s` }}
  >
    <div className="p-3 sm:p-4 bg-gradient-to-br from-cyan-500/20 to-blue-500/20 backdrop-blur-md rounded-xl sm:rounded-2xl border border-cyan-400/30 shadow-[0_0_30px_rgba(6,182,212,0.3)] hover:shadow-[0_0_50px_rgba(6,182,212,0.5)] transition-all duration-300">
...
export const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-950 text-slate-100">
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-blue-950/30 to-slate-900">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(6,182,212,0.1),transparent_50%)]" />
        <div
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(50)].map((_, i) => (
          <div
            key={i}
            className="absolute w-0.5 h-0.5 sm:w-1 sm:h-1 bg-cyan-400 rounded-full animate-particle"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
              animationDelay: `${Math.random() * 5}s`,
      ...
      {/* Floating icons */}
      <FloatingIcon Icon={Code} delay={0} position="top-16 left-4 sm:top-20 sm:left-10 lg:left-20" />
      <FloatingIcon Icon={Database} delay={0.5} position="top-40 right-6 sm:right-10 lg:right-32" />
      <FloatingIcon Icon={Globe} delay={1} position="bottom-28 left-8 sm:left-16 lg:left-40" />
      <FloatingIcon Icon={Cpu} delay={1.5} position="bottom-16 right-10 sm:right-20 lg:right-48" />

      {/* Main content */}
      <div className="relative z-10 text-center px-4 sm:px-6 md:px-10 max-w-5xl mx-auto">
        <div className="animate-fade-in-up">
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold mb-6 bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text text-transparent">
            Amol Shende
          </h1>

          <div className="h-1 w-24 sm:w-32 bg-gradient-to-r from-cyan-400 to-blue-500 mx-auto mb-8 rounded-full shadow-[0_0_20px_rgba(6,182,212,0.6)]" />

          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold mb-6 text-slate-200">
            Full-Stack Developer
          </h2>

          <p className="text-base sm:text-lg md:text-xl text-slate-300 mb-10 sm:mb-12 max-w-3xl mx-auto leading-relaxed">
            Crafting seamless digital experiences with modern web technologies.
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center items-center animate-slide-up">
            <button
              onClick={() => scrollToSection('projects')}
              className="group px-6 py-3 sm:px-8 sm:py-4 bg-gradient-to-r from-cyan-500 to-blue-500 rounded-full font-semibold text-lg hover:shadow-[0_0_40px_rgba(6,182,212,0.6)] transition-all duration-300 hover:scale-105 flex items-center gap-2"
            >
              View My Work
              <Globe className="w-5 h-5 group-hover:rotate-12 transition-transform" />
            </button>
            ...
            <a
              href="/AmolS.cv.pdf"
              download="AmolS.cv.pdf"
              className="group px-6 py-3 sm:px-8 sm:py-4 bg-slate-800/50 backdrop-blur-md border-2 border-cyan-400/50 rounded-full font-semibold text-lg hover:bg-cyan-400/10 hover:shadow-[0_0_30px_rgba(6,182,212,0.4)] transition-all duration-300 hover:scale-105 flex items-center gap-2"
            >
              <Download className="w-5 h-5 group-hover:translate-y-1 transition-transform" />
              Download Resume
            </a>
